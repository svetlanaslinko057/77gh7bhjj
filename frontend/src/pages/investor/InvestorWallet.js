...
export default function InvestorWallet() {
  ...
  const [meta, setMeta] = useState({ min_withdrawal_uah: 1000, currencies: ['UAH'] });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setMeta({
        ...
        currencies: w.data?.currencies || ['UAH'],
      });
                    ...
                    <div key={w.id} data-testid={`withdrawal-${w.id}`}
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="text-lg font-bold tabular-nums">{Number(w.amount).toLocaleString('uk-UA')} {w.currency}</p>
          ...
          <div className="grid grid-cols-3 gap-2">
            <label className="block col-span-2">
              ...
              <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)}
                data-testid="input-amount" min={minAmount} step="0.01"
                placeholder={`мін. ${minAmount}`}
