...
function BrowseTab() {
  ...
  const submit = async (mode) => {
    ...
    try {
      ...
      alert(mode === 'buy_now'
        ? `Угоду закрито! Отримано ${r.data?.trade?.units_uah?.toFixed(2)} UAH частки.`
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-2" onClick={() => setSelected(null)}>
          <div className="bg-card rounded-t-2xl sm:rounded-2xl w-full max-w-md p-5" onClick={e => e.stopPropagation()} data-testid="buy-modal">
            ...
            <div className="text-xs text-token-muted mb-3">
              Доступно: <strong>{formatUAH((selected.units_uah || 0) - (selected.filled_units_uah || 0))}</strong> · Ціна за 1 ₴ частки: <strong>{(selected.price_per_unit || 1).toFixed(3)}</strong>
            </div>
            <label className="block text-xs text-token-muted mb-1">Сума частки (UAH)</label>
            ...
            <details className="mt-2">
              ...
              <div className="mt-2 space-y-2">
                <input type="number" step="0.001" placeholder="Ціна за 1 UAH" value={offerPrice}
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-2" onClick={() => setSelected(null)}>
          <form onSubmit={submit} className="bg-card rounded-t-2xl sm:rounded-2xl w-full max-w-md p-5" onClick={e => e.stopPropagation()} data-testid="list-form">
            ...
            <label className="block text-xs text-token-muted mb-1">Ціна за 1 UAH частки (1.0 = номінал)</label>
