  ...
  return (
    <div className="p-6 md:p-10" data-testid="admin-ledger">
      ...
      <div className="grid sm:grid-cols-4 gap-3 mb-6">
        <SummaryCard label="Credit (UAH)" value={formatUAH(summary.total_uah_credit)} icon={<ArrowDownCircle className="w-5 h-5 text-emerald-600" />} testid="sum-credit" />
        <SummaryCard label="Debit (UAH)" value={formatUAH(summary.total_uah_debit)} icon={<ArrowUpCircle className="w-5 h-5 text-red-500" />} testid="sum-debit" />
        <SummaryCard label="Net (UAH)" value={formatUAH(summary.net_uah)} testid="sum-net" />
          ...
          <table className="w-full text-sm">
            <thead className="text-xs uppercase tracking-widest text-token-muted bg-muted/40">
              <tr>
                ...
                <th className="text-right px-4 py-3 font-medium">UAH</th>
              </tr>
            </thead>
            <tbody>
              {items.map((e) => (
