...
function CreatePlanModal({ assets, onClose, onCreated, onError }) {
  ...
  const submit = async () => {
    ...
    try {
      await lumen.post('/admin/payout-plans', {
        asset_id: assetId, type, frequency, expected_amount: amt, notes: notes.trim() || null,
      });
      onCreated();
  ...
  return (
    <Drawer title="Новий план нарахувань" onClose={onClose} testid="create-plan-modal">
      ...
      <label className="block">
        <span className="text-xs uppercase tracking-widest text-muted-foreground">Сума за період (₴)</span>
        <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} data-testid="plan-amount"
          placeholder="напр. 150000" className="mt-1 w-full h-11 rounded-lg border border-border bg-background px-3 text-sm" />
      </label>
      <label className="block">
