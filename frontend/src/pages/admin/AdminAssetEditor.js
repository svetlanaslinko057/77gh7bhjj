  ...
  return (
    <div className="p-6 md:p-10 max-w-3xl mx-auto" data-testid="admin-asset-editor">
      ...
      <section className="mt-8 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Мінімум входу, ₴" type="number" value={form.min_ticket} onChange={(v) => set('min_ticket', v)} />
          <Field label="Цільова сума раунду, ₴" type="number" value={form.round_target} onChange={(v) => set('round_target', v)} />
        </div>
      ...
      <section className="mt-10 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Ставка податку (ПДФО+ВЗ, 0–1)" type="number" value={form.tax_rate} onChange={(v) => set('tax_rate', v)} placeholder="0.195" testid="asset-tax-rate" />
...
function EconomicsPreview({ form }) {
  ...
  return (
    <div className="rounded-xl border p-4 mt-2" style={{ borderColor: 'var(--token-border)', background: 'rgba(46,93,79,0.04)' }} data-testid="economics-preview">
      ...
      <p className="text-xs text-token-muted mt-1">
        На тікет {econ.ticket.toLocaleString('uk-UA')} ₴ за {econ.horizon_years} р.
      </p>
      <div className="grid grid-cols-3 gap-3 mt-3 text-sm">
        ...
        <div>
          ...
          <p className="font-bold">{econ.annual.rental_net.toLocaleString('uk-UA')} ₴</p>
        </div>
        <div>
          <p className="text-[10px] uppercase tracking-widest text-token-muted">Сумарно</p>
          <p className="font-bold">{econ.totals.total_net.toLocaleString('uk-UA')} ₴</p>
        </div>
      </div>
    </div>
  );
