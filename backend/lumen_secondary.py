...
async def _settle_trade(trade: dict, *, actor: dict,
    ...
    # 5. Listing fill-tracking
    new_filled = _round2(float(listing.get("filled_units_uah") or 0) + units_uah)
    remaining = _round2(float(listing.get("units_uah") or 0) - new_filled)
    if remaining <= 0.5:
        listing_status = "filled"
    else:
        listing_status = "partially_filled"
    await db.lumen_secondary_listings.update_one(
        {"id": listing["id"]},
        {"$set": {"filled_units_uah": new_filled, "status": listing_status,
                  "updated_at": _now(),
                  **({"filled_at": _now()} if listing_status == "filled" else {})}},
    )
    ...
    await write_audit(
        ...
        summary=(f"Trade {tid} settled: {units_uah:.2f} UAH @ price={trade['price_per_unit']}"
                 f" between {seller_id} → {buyer_id}, fee={fee_uah:.2f}"),
...
async def create_listing(payload: CreateListingPayload, request: Request,
    ...
    if payload.units_uah < MIN_LISTING_UAH:
        raise HTTPException(status_code=400,
                            detail=f"Мінімум для виставлення: {MIN_LISTING_UAH:.0f} UAH")
    ...
    if free + 0.5 < payload.units_uah:
        raise HTTPException(status_code=400,
                            detail=f"Доступно для продажу: {free:.2f} UAH")
    ...
    await write_audit(
        ...
        summary=f"Listing created: {L['units_uah']:.2f} UAH of {payload.asset_id} @ {payload.price_per_unit}",
...
async def create_bid_or_buy(payload: CreateBidPayload, request: Request,
    ...
    if payload.units_uah > remaining + 0.5:
        raise HTTPException(status_code=400,
                            detail=f"Доступно для купівлі: {remaining:.2f} UAH")
    ...
    if is_buy_now:
        ...
        if avail + 0.5 < gross:
            raise HTTPException(
                ...
                detail=(f"Недостатньо коштів на гаманці: потрібно "
                        f"{gross:.2f} UAH, доступно {avail:.2f} UAH"),
            )
        trade = {
        ...
        await write_audit(
            ...
            actor=user, request=request,
            summary=f"Buy-now trade created: {gross:.2f} UAH on listing {L['id']}",
        )
    ...
    await write_audit(
        ...
        actor=user, request=request,
        summary=f"Bid placed: {bid['units_uah']:.2f} UAH @ {price} on listing {L['id']}",
    )
...
async def accept_bid(bid_id: str, request: Request,
    ...
    await write_audit(
        ...
        target_type="lumen_secondary_bids", target_id=bid_id,
        actor=user, request=request,
        summary=f"Bid {bid_id} accepted, trade {trade['id']} created ({gross:.2f} UAH)",
    )
    settled = await _settle_trade(trade, actor=user, request=request)
