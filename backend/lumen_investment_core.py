...
async def create_intent(user: dict, asset_id: str, amount: float,
    ...
    if amount < min_ticket:
        raise HTTPException(status_code=400, detail=f"Мінімальна сума: {min_ticket:.0f} ₴")
    if round_id:
    ...
    await _notify(
        ...
        f"Заявка на {amount:,.0f} ₴ у «{asset.get('title')}» подана. "
...
async def approve_intent(intent_id: str, payload: IntentDecisionPayload = None,
    ...
    if kyc_ok:
        await _notify(
            ...
            f"Інвестицію {amount:,.0f} ₴ у «{asset.get('title')}» зарезервовано. "
