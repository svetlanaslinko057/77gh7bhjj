...
async def _save_upload(subdir: str, file: UploadFile) -> tuple[str, str, int]:
    ...
    if ct and ct not in _ALLOWED_DOC_CT:
        raise HTTPException(status_code=400,
                            detail="Дозволені формати: PDF, DOC(X), XLS(X), PNG, JPG, WEBP")
    folder = os.path.join(_UPLOAD_ROOT, subdir)
    os.makedirs(folder, exist_ok=True)
    safe_name = re.sub(r"[^\w.\-]+", "_", file.filename or "file")[:120]
    path = os.path.join(folder, f"{uuid.uuid4().hex[:10]}_{safe_name}")
    with open(path, "wb") as fh:
        fh.write(content)
    return path, (file.filename or safe_name), len(content)
        ...
        try:
            ...
            pdf_val = _make_seed_pdf(
                ...
                ["Об'єкт: ЖК «Подільський», Київ, Поділ",
                 ...
                 "Ринкова вартість завершеного об'єкта: 96.4 млн ₴",
